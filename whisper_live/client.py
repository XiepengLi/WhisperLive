...
class Client:
    ...
    def __init__(
        ...
        self.last_segments = [{"start": ""}]
        self.stable_segments = [{"start": ""}]
    ...
    def on_message(self, ws, message):
        ...
        if "segments" in message.keys():
            if (
                self.last_segments[-1] == message["segments"][-1]
                and self.last_segments[-1]["start"] != self.stable_segments[-1]["start"]
            ):
                self.stable_segments.append(self.last_segments[-1])
            utils.clear_screen()
            print(message["segments"][-1])
            print(self.stable_segments)
            self.process_segments(message["segments"])
            self.last_segments = message["segments"]
